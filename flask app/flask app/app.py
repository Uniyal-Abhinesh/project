...
app.config['MYSQL_PASSWORD'] = ''  # empty string for XAMPP default
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                print(f"[DEBUG] Password hash in DB: {user[0]}")
                print(f"[DEBUG] Password check: {check_password_hash(user[0], password)}")
